...
function inicializarTimerApp() {
  ...
  let tempoFoco = obterDado('pomodoro_focus_time', 1500);
  let tempoPausa = obterDado('pomodoro_break_time', 300);
  ...
  let tempoRestante = obterDado('pomodoro_tempo_restante', tempoFoco);
  ...
  if (settingsForm) {
    settingsForm.addEventListener('submit', (evento) => {
      ...
      if (isNaN(novoTempoFoco) || novoTempoFoco < 1 || novoTempoFoco > 3600) {
        mostrarNotificacao('Tempo de foco deve ser entre 1 e 3600 segundos.', 'warning');
        return;
      }

      if (isNaN(novoTempoPausa) || novoTempoPausa < 1 || novoTempoPausa > 1800) {
        mostrarNotificacao('Tempo de pausa deve ser entre 1 e 1800 segundos.', 'warning');
        return;
      }
